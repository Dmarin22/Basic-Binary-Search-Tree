...
public class BasicBST<Key extends Comparable<Key>, Value> {
    ...
    public int height(Key key) {
        ...
        return new_height(root, key);
    }

    private int new_height(Node y, Key key) {
        ...
        if (cmp < 0) {
            return new_height(y.left, key);
        }
        if (cmp > 0) {
            return new_height(y.right, key);
        }
        return 1 + Math.max(height(y.left), height(y.right));
    }
    ...
    private void in_order(Node y, Queue<Key> queue) {
        if (y == null) {
            return;
        }
        ...
        in_order(y.left, queue);
        in_order(y.right, queue);
    }
